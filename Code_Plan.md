...
각 user_id별 최신 row를 기준으로 KR profile 초기화를 수행한다.
...
### 6. Belief fallback
belief similarity 계산 시 최신 forum post나 belief 텍스트가 없으면  
`util/belief/belief_100_kr.csv`의 초기 belief를 fallback으로 사용한다.  
그래도 없으면 빈 문자열이 아니라 neutral default belief 문장을 사용한다.
...
### 8. Hold 및 빈 주문 처리
`hold`는 완전히 유효한 결정이다.  
LLM이 hold를 선택하면 빈 주문 리스트를 정상 결과로 처리한다.  
빈 주문 때문에 에러가 발생하지 않도록 parser와 downstream validation을 확인한다.
...
KR 뉴스 원천 파일은 `data/samsung_news.pkl`로 저장한다.  
...
Community OFF는 `logs_kr_off`, Community ON은 `logs_kr_on`을 사용한다.

### 13. 실험 기간
뉴스 데이터 기간에 맞춰 Smoke test는 `2026-02-02`부터 `2026-02-06`까지로 한다.  
본 실험은 `2026-02-02`부터 `2026-03-31` 사이의 거래일을 사용한다.
...
### 16. hold 표준 출력 스키마
hold는 빈 주문 리스트로 처리한다.  
표준 출력은 가능한 경우 기존 파서 구조를 유지하되, 거래 주문은 `orders: []`로 둔다.  
필수 action 필드가 필요한 코드 경로에서는 `action: "hold"`와 `orders: []`를 함께 사용한다.  
downstream에서는 `orders: []`를 정상적인 무거래 결과로 처리해야 한다.

### 17. 뉴스 데이터 정책
뉴스 데이터는 새로 스크래핑하지 않는다.  
사용자가 이미 수집한 `samsung_mk_news_20260201_20260331.pkl` 파일을 기준 뉴스 데이터로 사용한다.

해당 파일은 삼성전자 관련 MK 뉴스 데이터이며, 기간은 `2026-02-01`부터 `2026-03-31`까지이다.

KR PoC에서는 이 파일을 `data/samsung_news.pkl`로 복사 또는 변환하여 사용한다.  
필요 시 pandas 버전 호환 문제를 고려하여 로딩/재저장 스크립트를 작성한다.

기대 포맷은 기존 TwinMarket 뉴스 pkl과 동일하게 유지한다:

- `cal_date`: YYYY-MM-DD 형식 문자열
- `news`: 해당 날짜 뉴스 리스트

Stage 1-5의 `build_kr_news.py`는 웹 스크래핑이 아니라,
기존 pkl 파일을 검증하고 `data/samsung_news.pkl` 및 `data/InformationDB_samsung/`를 생성하는 스크립트로 변경한다.
